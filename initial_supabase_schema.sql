...
CREATE POLICY "Users can create their own claims." ON public.user_claims FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view their own claims." ON public.user_claims FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can update their own claims." ON public.user_claims FOR UPDATE USING (auth.uid() = user_id);
